...
class Ui_AddDialog(QtGui.QDialog):
    ...
    def retranslateUi(self, AddDialog):
        AddDialog.setWindowTitle(_translate("AddDialog", "Add an Entry", None))
    ...
    def CollectData(self):
        ...
        if self.Functionhasbeencalled == "True":
            ...
            self.Functionhasbeencalled = "False"

        else:
            programdirectory = str(Path.cwd())+ "\\Images\\"+"Missingimage.jpg"

        DbManager.db.AddEntryToDataBase(self.TitleData,self.typecombobox,self.AuthorStudioData,self.GenreData,self.VolumeData,self.ChaptersData,self.statuscombobox,programdirectory)
